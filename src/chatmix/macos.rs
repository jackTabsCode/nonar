...
impl ChatMixBackend for ChatMix {
    fn new(_output_name: &'static str) -> Result<Self, ChatMixError> {
        let devices = list_audio_devices()?;

        let game_id = find_device_by_name(&devices, GAME_SINK_NAME).ok_or_else(|| {
            ChatMixError::SinkNotFound {
                sink: GAME_SINK_NAME.to_string(),
            }
        })?;
        let chat_id = find_device_by_name(&devices, CHAT_SINK_NAME).ok_or_else(|| {
            ChatMixError::SinkNotFound {
                sink: CHAT_SINK_NAME.to_string(),
            }
        })?;
...
fn list_audio_devices() -> Result<Vec<AudioObjectID>, ChatMixError> {
...
fn set_device_volume(device: AudioObjectID, vol: u8) -> Result<(), ChatMixError> {
...
fn get_property_data<T>(
...
) -> Result<Vec<T>, ChatMixError> {
    let mut size = 0;
    let status = unsafe { AudioObjectGetPropertyDataSize(object, addr, 0, ptr::null(), &mut size) };

    if status != 0 {
        return Err(ChatMixError::CoreAudioError(format!(
            "Failed to get property data size: {status}"
        )));
    }
    ...
    if status != 0 {
        return Err(ChatMixError::CoreAudioError(format!(
            "Failed to get property data: {status}"
        )));
    }
...
fn set_property_data<T>(
    ...
    if status != 0 {
        return Err(ChatMixError::CoreAudioError(format!(
            "Failed to set property data: {status}"
        )));
    }
